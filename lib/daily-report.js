...
function formatText(report) {
  ...
  lines.push("*Sentry*");
  const se = report.sentry.summary;
  lines.push(`• Unresolved issues: ${se.unresolvedIssues} (${se.highPriority} high priority, ${se.errorEvents} events in last 24h)`);
  if (se.highPriority > 0) {
    lines.push(`• High-priority issues (${se.highPriority}): Dev is actively looking into these — not unattended.`);
  }
  ...
  lines.push("*Ahrefs*");
  const ahrefsOk = report.ahrefs.domainRating != null || report.ahrefs.siteAuditProjects.length > 0;
  const primaryProject = report.ahrefs.siteAuditProjects[0];
  if (!ahrefsOk) {
    lines.push("• Ahrefs data could not be loaded for this run.");
  }
  if (report.ahrefs.ratingDate && report.ahrefs.ratingDate !== report.reportDate) {
    lines.push(`• Domain rating date: ${report.ahrefs.ratingDate} (latest available from Ahrefs)`);
  }
  lines.push(`• ${report.ahrefs.target}: DR ${report.ahrefs.domainRating ?? "—"} · Rank ${report.ahrefs.ahrefsRank ?? "—"}`);
  if (primaryProject) {
    lines.push(`• Primary site audit: ${primaryProject.name} — health score ${primaryProject.healthScore ?? "—"}${primaryProject.urlsWithErrors != null ? `, ${primaryProject.urlsWithErrors} URLs with errors` : ""}`);
  }
  ...
  if (report.ahrefs.siteAuditProjects.length > 1) {
    lines.push(`• All site audit projects (${report.ahrefs.siteAuditProjects.length}):`);
  ...
  lines.push("");
  lines.push("*Slack summary (factual — report numbers only, no verdicts)*");
  let sentryDigest = `SENTRY: ${se.unresolvedIssues} unresolved issues (${se.highPriority} high priority, ${se.errorEvents} events in last 24h)`;
  if (se.highPriority > 0) sentryDigest += " — Dev is looking into high-priority items";
  lines.push(sentryDigest);
  lines.push(`CHECKLY: ${cs.passing}/${cs.total} checks passing, ${cs.failing} failing, ${cs.degraded} degraded`);
  if (ahrefsOk) {
    lines.push(`AHREFS: DR ${report.ahrefs.domainRating ?? "—"}, rank ${report.ahrefs.ahrefsRank ?? "—"}${primaryProject ? `, ${primaryProject.name} health ${primaryProject.healthScore ?? "—"}/100` : ""}`);
  } else {
