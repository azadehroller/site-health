...
// New Sentry orgs (including roller-ja) store frontend performance on the
// spans dataset. The old Discover query `event.type:transaction` returns empty.
const PAGELOAD = "span.op:[pageload,navigation]";

async function sentryEvents(fields, { query = "", sort, per_page } = {}) {
  const params = {
    dataset: "spans",
    field: fields,
    query,
  ...
  return sentryFetch("/api/0/organizations/{org}/events/", params);
}

async function sentryCount(query) {
  const data = await sentryEvents(["count()"], { query });
  return num(rows(data)[0], "count()");
}

async function sentrySlowestPages() {
  try {
    return await sentryEvents(
      ["transaction", "p95(span.duration)", "count()"],
      { query: PAGELOAD, sort: "-p95(span.duration)", per_page: 5 }
    );
  } catch {
    return sentryEvents(
      ["transaction", "p95(transaction.duration)", "count()"],
      { query: PAGELOAD, sort: "-p95(transaction.duration)", per_page: 5 }
    );
  }
}

async function sentryApdexPages() {
  try {
    return await sentryEvents(
      ["transaction", "failure_rate()", "apdex(3000)", "count()"],
      { query: PAGELOAD, sort: "-count()", per_page: 5 }
    );
  } catch {
    const data = await sentryEvents(
      ["transaction", "count()", "count_if(span.duration,greater,3000)", "count_if(span.duration,greater,12000)"],
      { query: PAGELOAD, sort: "-count()", per_page: 5 }
    );
    return {
      data: rows(data).map((r) => {
        const total = num(r, "count()") || 0;
        const over3s = num(r, "count_if(span.duration,greater,3000)") || 0;
        const over12s = num(r, "count_if(span.duration,greater,12000)") || 0;
        const satisfied = Math.max(total - over3s, 0);
        const tolerating = Math.max(over3s - over12s, 0);
        return {
          transaction: r.transaction,
          "apdex(3000)": total ? (satisfied + 0.5 * tolerating) / total : null,
          "failure_rate()": total ? over3s / total : null,
          "count()": total
        };
      })
    };
...
async function fetchSentryReport() {
  const [cwvAgg, inpAgg, cwvPages, slow, apdex, revenueJourneys, paidLpJourneys] = await Promise.all([
    sentryEvents(
      ["p75(measurements.lcp)", "p75(measurements.fcp)", "p75(measurements.cls)", "p75(measurements.inp)", "p75(measurements.ttfb)"],
      { query: `${PAGELOAD} OR has:measurements.lcp` }
    ),
    sentryEvents(["p75(measurements.inp)"], { query: "has:measurements.inp" }),
    sentryEvents(
      ["transaction", "p75(measurements.lcp)", "p75(measurements.inp)", "p75(measurements.cls)", "p75(measurements.fcp)", "count()"],
      { query: `${PAGELOAD} has:measurements.lcp`, sort: "-count()", per_page: 10 }
    ),
    sentrySlowestPages(),
    sentryApdexPages(),
    sentryCount(
      `${PAGELOAD} (transaction:/get-started* OR transaction:/pricing* OR transaction:/checkout* OR transaction:/signup* OR transaction:/demo* OR transaction:/contact*)`
    ),
    sentryCount(
      `${PAGELOAD} (transaction:*/features/* OR transaction:*/industries/* OR transaction:*/pricing* OR transaction:*/venue-management*)`
    )
  ]);

  const cwv = rows(cwvAgg)[0] || {};
  const inp = rows(inpAgg)[0] || {};
  const apdexRows = rows(apdex);
  const siteApdex = apdexRows.find((r) => r.transaction === "/") || apdexRows[0];

  return {
    webVitals: {
      lcpMs: num(cwv, "p75(measurements.lcp)"),
      fcpMs: num(cwv, "p75(measurements.fcp)"),
      cls: num(cwv, "p75(measurements.cls)"),
      inpMs: num(cwv, "p75(measurements.inp)") ?? num(inp, "p75(measurements.inp)"),
      ttfbMs: num(cwv, "p75(measurements.ttfb)")
    },
    performance: {
      siteApdex: num(siteApdex, "apdex(3000)"),
      siteFailureRate: num(siteApdex, "failure_rate()"),
      siteSamples: num(siteApdex, "count()")
    },
    cwvByPage: rows(cwvPages).map((r) => ({
      page: r.transaction,
      views: num(r, "count()"),
      lcpMs: num(r, "p75(measurements.lcp)"),
      fcpMs: num(r, "p75(measurements.fcp)"),
      cls: num(r, "p75(measurements.cls)"),
      inpMs: num(r, "p75(measurements.inp)")
    })),
    slowestPages: rows(slow).map((r) => ({
      page: r.transaction,
      p95DurationMs: num(r, "p95(span.duration)", "p95(transaction.duration)"),
      samples: num(r, "count()")
    })),
    apdex: apdexRows.map((r) => ({
      page: r.transaction,
      apdex: num(r, "apdex(3000)"),
...
function formatText(report) {
  ...
  if (!ahrefsOk) {
    ...
    if (ah.domainRating != null || ah.ahrefsRank != null) {
  ...
  if (ahrefsOk && projects.length) {
    ...
    const drPart = ah.domainRating != null ? `Domain Rating ${ah.domainRating}, rank ${ah.ahrefsRank ?? "—"}, ` : "";
    lines.push(`AHREFS: ${drPart}${top.name} health ${top.healthScore ?? "—"}/100, ${top.urlsWithErrors ?? "—"} URLs with errors`);
  } else if (ahrefsOk && ah.domainRating != null) {
...
async function buildDailyReport() {
  ...
  if (ahrefsRaw?.partialErrors?.length && !ahrefs.dataAvailable) {
    ahrefsRaw.partialErrors
      .filter((msg) => !String(msg).includes("bad date"))
      .forEach((message) => errors.push({ source: "ahrefs", message }));
  }
